...
export default function EditTask({ task }: { task: Task }) {
  ...
  const [[taskTitle, taskDescription], setTask] = useState([task.title, task.description]);
      ...
      {isOpen && (
        <div onClick={() => setIsOpen(false)} className="fixed top-0 bottom-0 w-full right-0 bg-black/40 grid place-items-center px-6 overflow-y-scroll py-6 z-30">
          <div onClick={e => e.stopPropagation()} className="bg-white dark:bg-dark-grey w-full p-4 rounded-lg md:max-w-[450px]">
            <h2 className="text-headingL font-bold">Edit Task</h2>
            <div className="mt-4">
            ...
            <div className="mt-4">
              ...
              <button
                onClick={() => setSubtasks([...subtasks, {id: randomKey(), title: "", isCompleted: false}])}
